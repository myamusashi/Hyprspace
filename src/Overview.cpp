        ...
        for (auto& ws : g_pCompositor->m_vWorkspaces) {
            if (ws->m_iMonitorID == ownerID) {
                ...
                if (w != nullptr && ws->m_efFullscreenMode != FSMODE_NONE) {
                    ...
                    if (ws->m_efFullscreenMode == FSMODE_FULLSCREEN) w->m_bCreatedOverFullscreen = true;
                    ...
                    // g_pCompositor->setWindowFullscreenState(w, false);
                }
            }
        }
    }
...
void CHyprspaceWidget::hide() {
    ...
    for (auto& fs : prevFullscreen) {
        ...
        g_pCompositor->changeWindowFullscreenModeClient(w, oFullscreenMode, true); 
